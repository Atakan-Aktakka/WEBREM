...
const CategoryList = () => {
  ...
  const posts = useSelector((state) => state?.post?.postLists);

  let { categoryList, loading, appErr, serverErr } = category;

  // set post data.
  if (posts?.length > 0 && categoryList?.length > 0) {
    ...
    lists.forEach((list) => {
      let count = 0;
      let total = 0;
      allPost.forEach((category) => {
        if (list.title.toLowerCase() == category.category.toLowerCase()) {
          count = count + category.numViews;
          total = total + 1;
        }
      });
      const newObj = { ...list };
      newObj["views"] = count;
      newObj["totalPost"] = total;
      updatedPosts.push(newObj);
    });
    categoryList = [...updatedPosts];
  }
